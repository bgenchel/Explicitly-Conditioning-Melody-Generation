...
class ChordInterBarPosCondLSTM(nn.Module):
    def __init__(self, vocab_size, embed_dim, cond_vocab_size, cond_embed_dim, output_dim, hidden_dim=128,
        ...
        encoder_layers = [
            nn.Linear(embed_dim + cond_embed_dim + const.BARPOS_EMBED_DIM + 2 * const.CHORD_EMBED_DIM, hidden_dim),
            nn.ReLU()]
        if batch_norm:
    ...
    def forward(self, data):
        x, conds, barpos, chord_roots, chord_pcs, nxt_chord_roots, nxt_chord_pcs = data
        ...
        encoding = self.encoder(torch.cat([x_embeds, cond_embeds, barpos_embeds, chord_embeds, nxt_chord_embeds], 2)) 
...
class PitchLSTM(ChordInterBarPosCondLSTM):
    ...
    def data_assembler(self, data_dict):
        ...
        return (x, cond, barpos, chord_roots, chord_pcs, nxt_chord_roots, nxt_chord_pcs)
