...
def convert_harmony_to_piano_roll_mat(chords, dur_nums):
    dur_ticks = [TAG_TO_TICKS[NUM_TO_TAG[dur]] for dur in dur_nums]
    onsets = np.array([np.sum(dur_ticks[:i]) for i in range(len(dur_ticks))])
    total_ticks = sum(dur_ticks)
    output_mat = np.zeros([128, int(total_ticks)])
...
seed_durs = []
seed_note_chords = []
for i in range(args.seed_measures):
    measure_pitches = seed_song['measures'][i]['pitch_numbers']
    measure_durs = seed_song['measures'][i]['duration_tags']
    measure_note_chords = []
    harmony_index = 0
    for j in range(len(seed_pitches)):
