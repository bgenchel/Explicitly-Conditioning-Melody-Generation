...
for measure in seed_song['measures']:
    ...
    harmony_index = 0
    for i in range(seed_song_pitches):
        if (i == measure['half_index']) and (len(measure['harmonies']) > 1):
            harmony_index += 1
        seed_song_chords.append(measure['harmonies'][harmony_index])

seed_pitches = seed_song_pitches[:SEED_LEN]
pitch_inpt = Variable(torch.LongTensor(seed_pitches)).view(1, -1)
seed_durs = seed_song_durs[:SEED_LEN]
dur_inpt = Variable(torch.LongTensor(seed_durs)).view(1, -1)
seed_chords = seed_song_chords[:SEED_LEN]
chord_inpt = Variable(torch.FloatTensor(seed_chords)).view(1, -1)

pitch_seq = seed_pitches
chord_seq = seed_chords
dur_seq = seed_durs
for i in range(args.seq_len):
    pitch_out = pitch_net(pitch_inpt, chord_inpt)
    pitch_seq.append(np.argmax(pitch_out.data[:, -1, :]))
    pitch_inpt = Variable(torch.LongTensor(pitch_seq[-2:]).view(1, -1))

    chord_seq.append(seed_chords[SEED_LEN + i])
    chord_inpt = Variable(torch.FloatTensor(chord_seq[-2:]).view(1, -1))
