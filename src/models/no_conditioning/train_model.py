...
try:
    ...
    init_train_loss = trn.eval_epoch(model, train_loader, assembler, assembler, loss_fn)  
    train_losses.append(init_train_loss)
    print("Initial Training Loss: %.5f" % (init_train_loss))
    init_valid_loss = trn.eval_epoch(model, valid_loader, assembler, assembler, loss_fn)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = trn.train_epoch(model, train_loader, assembler, assembler, loss_fn, optimizer)
        train_losses.append(train_loss)
        print("Epoch %d Training Loss: %.5f" % (epoch_label, train_loss))
        valid_loss = trn.eval_epoch(model, valid_loader, assembler, assembler, loss_fn)
